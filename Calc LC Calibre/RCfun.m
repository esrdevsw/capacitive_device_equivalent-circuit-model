% função real para ponto 1 e para 3 das 4 variaveis
% A = Lcpar;
% B = Rhopar;
% C = massa
function [Y1r, Y1c, Tta] = RCfun(LCmed, rhob, massa, Prss,lmbd, frequ, HM)
...
ms = massa; %VetPt(NP,4); % massa
% Prss = VetPt(NP,3); % pressao

% propriedades cristalograficas do LaNi5
DensTT = VolumeLaNi5(HM); %g/cm3

% volume do solido pela densidade (m3)
Vs = (ms./DensTT).*10^-6; % ms é a massa da amostra

% volume do anel total ( Lc aLcura total da amostra)
Vt = pi.*LC.*(((7.5001.*mm).^2) - ((5.7.*mm).^2)); %cm3
